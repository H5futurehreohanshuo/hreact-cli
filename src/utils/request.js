...
import { commonUrl, showAlert, Toast } from './tool'; // 引入加密算法
...
export const request = (data, url, cb, method = 'GET', responseType = 'json') => {
  ...
  if (method === 'POST' || method === 'post') { // post 请求
    ...
    httpUrl = `${commonUrl}${url}?${stringify(data)}`;
    httpInit = {
      method,
    };
  }
...
// 实例代码
/* encryptRequest(
  ['departure','arrivals','start','end'],['BJS', 'SHA', '2017-05-03', '2018-05-03'],
  '/h5/app/pricecalendar',
  function (res) {
  console.log(res);
}); */
export const encryptRequest = (keyArr, valArr, url, cb, method = 'GET') => {
  ...
  const aesData = aes.aesEncrypt(keyArr, valArr); // 对参数进行加密
  if (method === 'POST' || method === 'post') { // post 请求
    httpUrl = `${commonUrl}${url}`;
    httpInit = {
      method,
      body: stringify(aesData[0]),
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
      },
    };
  } else { // get 请求
    httpUrl = `${commonUrl}${url}?${stringify(aesData[0])}`;
    httpInit = {
      method,
    };
  }
